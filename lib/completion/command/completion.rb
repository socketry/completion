...
module Completion
	module Command
		# Generate or install shell completion adapter scripts.
		class Completion < Samovar::Command
			...
			class Generate < Samovar::Command
				...
				def call
					output.puts ::Completion::Shell.script(shell: @options[:shell].to_sym, executable: @options[:command])
				end
			end
			...
			class Install < Samovar::Command
				...
				def call
					...
					script = ::Completion::Shell.script(shell: shell.to_sym, executable: @options[:command])
